...
#                           <macan@ncic.ac.cn>
#
# Time-stamp: <2012-11-23 11:50:05 macan>
#
...
if [ -e $HVFS_HOME/conf/client.conf ]; then
    ...
    RC_CMD="LD_LIBRARY_PATH=$HVFS_HOME LOG_DIR=$LOG_DIR "`echo $ARGS`
fi
...
function start_client() {
    if [ "x$1" == "x" ]; then
        ...
        for x in $ipnr; do 
            ...
            $SSH $UN$ip "$RC_CMD $HVFS_HOME/ctdk_huadan_1d > $LOG_DIR/ctdk_huadan_1d.$id.log" > /dev/null &
        done
...
function start_osd() {
    if [ "x$1" == "x" ]; then
        ...
        for x in $ipnr; do 
            ...
            id=`echo $x | awk -F: '{print $2}'`
            port=`echo $x | awk -F: '{print $3}'`
            $SSH $UN$ip "$RS_CMD $HVFS_HOME/test/xnet/osd.ut $id $R2IP $port > $LOG_DIR/osd.$id.log" > /dev/null &
        done
